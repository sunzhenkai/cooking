...
switch (yykind)
    {
      ...
      {
        return WCParser::symbol_name (this->kind ());
      }
#endif // #if YYDEBUG || 0
    ...
    WCParser (WCScanner &scanner_yyarg, WCDriver &driver_yyarg);
    virtual ~WCParser ();

#if 201103L <= YY_CPLUSPLUS
    /// Non copyable.
    WCParser (const WCParser&) = delete;
    /// Non copyable.
    WCParser& operator= (const WCParser&) = delete;
#endif
...
#if YY_CPLUSPLUS < 201103L
    /// Non copyable.
    WCParser (const WCParser&);
    /// Non copyable.
    WCParser& operator= (const WCParser&);
#endif
